...
**How to explain it:** "CI used to authenticate with long-lived `AWS_ACCESS_KEY_ID`/`SECRET` GitHub Secrets. I replaced that with an OIDC federation: GitHub mints a short-lived token per workflow run, AWS validates it came from this exact repo and branch via the `sub` claim, and issues temporary credentials. There's no static credential sitting in GitHub to leak, rotate, or accidentally log."

**Likely questions:**
- *Why two roles instead of one?* Least privilege -- the app-deploy role can only push to the two ECR repos and update the two ECS services; it has zero IAM/VPC/RDS access. The terraform role is broader (it manages the whole stack) but is still scoped to the specific AWS services this project uses, not `AdministratorAccess`, and its blast radius is bounded by GitHub's environment-protection manual approval gate before it ever runs `apply`.
- *What stops someone from forking the repo and assuming the role?* The trust policy's `sub` condition matches `repo:SORABH13/secure-3-tier-Node-application:ref:refs/heads/master` exactly -- a fork has a different repo name, so its tokens fail the condition.
- *Isn't `iam:*` in the terraform role dangerous?* Yes, that's the honest tradeoff -- it's scoped to zero other AWS accounts and gated behind OIDC + manual approval, but a compromise of that specific role is still high-impact. The stronger version of this answer for a larger org would be per-resource ARNs generated from Terraform plan output (e.g. via a policy-as-code tool), which is more setup than this assignment's scope justified.
